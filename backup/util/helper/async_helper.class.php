...
class async_helper  {
    ...
    /**
     * Get all the current in progress async backups.
     *
     * @param string $filearea The filearea to get backup data for.
     * @param integer $contextlevel The contextlevel to get backup data for.
     * @param integer $instanceid The context id to get backup data for.
     * @return array $tabledata the rows of table data.
     */
    public static function get_async_backups($filearea, $contextlevel, $instanceid) {
        global $DB;
        ...
        if ($filearea == 'backup') {
            ...
            foreach ($records as $record) {
                $bc = \backup_controller::load_controller($record->backupid);  // Get the backup controller.

                // Get usefull info to render async status in correct area.
            ...
            if (($filearea == 'course' && $contextlevel == CONTEXT_COURSE)
            || ($filearea == 'activity' && $contextlevel == CONTEXT_MODULE)) {
     ...
     * @return [bool] Array of user data settings.
     */
    public static function get_userdata_backup_settings($backupcontroller) {
